...
async def calculate_design(data: Dict[str, Any]):
    ...
    try:
        ...
        print("\n" + "="*60)
        print("INCOMING CALCULATION REQUEST")
        print("="*60)
        print(f"Conductor: {data.get('conductor_data', {})}")
        print(f"Span Data: {data.get('span_data', {})}")
        print(f"Pole Geometry: {data.get('pole_geometry', {})}")
        print(f"Environmental: {data.get('environmental_data', {})}")
        print("="*60 + "\n")
